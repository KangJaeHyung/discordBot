...
public class VoiceService {
	...
	public static HashMap<VoiceChannelPk, VoiceChannelEntity> channel;
	...
	public void createGivenCreateChannel(SlashCommandInteractionEvent event) throws Exception {
		...
		if (voiceChannelRepository.findById(new VoiceChannelPk(channel.getGuild().getId(), channel.getId()))
				.isPresent()) {
		...
		VoiceService.channel.put(new VoiceChannelPk(channel.getGuild().getId(), channel.getId()), entity);
	...
	public void deleteGivenCreateChannel(SlashCommandInteractionEvent event) throws Exception {
		...
		VoiceService.channel.remove(new VoiceChannelPk(channel.getGuild().getId(), channel.getId()));
	}
...
//			
	...
	public void createBotChannel(SlashCommandInteractionEvent event) {
		TextChannel channel = event.getOption("channel").getAsChannel().asTextChannel();
		if (voiceChannelRepository.findByIdAndGiven(channel.getId(), Given.BOTCHAN).isPresent()) {
	...
	public HashMap<VoiceChannelPk, VoiceChannelEntity> findAll() {
		HashMap<VoiceChannelPk, VoiceChannelEntity> map = new HashMap<>();

		voiceChannelRepository.findAll().forEach(entity -> map.put(new VoiceChannelPk(entity.getGuildId(),entity.getId() ), entity));
		return map;
	}
}
